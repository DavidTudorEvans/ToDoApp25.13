...
//Fill modal inputs (prepopulate)
            ...
            //save changes (when user clicks save)
...
// Retrieving and displaying tasks from api.js file (tasks array) START
async function displayTasks() {

const sortBy = sortBtn.value; //dueDate, dateCReated, Default  User selection - send to backend
...
// this gets the tasks from api/tasks in the backend. See line 35 in backend index.js
try {
  ...
  const data = await response.json();
    ...
    function formatTask(task) {
        ...
        const done = task.completed ? "text-decoration-line-through opacity-50" : ""; //class list if task is completed or not (line through if done, nothing if not)
            ...
            <h4 class="${done} col-11">${task.title}</h4>
                <button data-id="${task._id}" type="button" class="btn-close delete" aria-label="Delete"></button>
                    ...
                    <div>
                        ...
                        <p class="m-0 class="${done}"><strong>Created on : </strong>${new Date(task.dateCreated).toLocaleDateString()}</p>
                    </div>
        `;
        return li;
    }
    toDoList.innerHTML = ""; //reset flash of the task status to clear the old status -refreshes the "To Do column"
...
// Updating the tasks (task not complete)
